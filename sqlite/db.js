// db.js
// 使用 sqlite3 操作資料庫，並初始化 movie_quotes table 及預設資料
...
db.serialize(() => {
    ...
    // 檢查是否已有資料，若無則插入預設資料
    db.get('SELECT COUNT(*) as count FROM movie_quotes', (err, row) => {
        if (err) {
        ...
        if (row.count === 0) {
            ...
            insert.finalize();
            console.log('已插入預設電影金句資料');
        } else {
            console.table('movie_quotes table 已有資料，未重複插入');
        }
    });
});
